...
import {ReferentielMetierManagerService} from 'src/app/shared/service/manager/profil/ReferentielMetierManager.service';
...
@Component({
  selector: 'app-referentiel-metier-edit-manager',
  standalone: false,
  templateUrl: './referentiel-metier-edit-manager.component.html'
})
export class ReferentielMetierEditManagerComponent implements OnInit {
    ...
    constructor(private service: ReferentielMetierManagerService , private metierService: MetierManagerService, private niveauLangueService: NiveauLangueManagerService, private langueService: LangueManagerService, @Inject(PLATFORM_ID) private platformId?) {
 ...
 public edit(): void {
   ...
   public async openCreateNiveauLangue(niveauLangue: string) {
        const isPermistted = await this.roleService.isPermitted('NiveauLangue', 'edit');
        if (isPermistted) {
   ...
   public async openCreateMetier(metier: string) {
        const isPermistted = await this.roleService.isPermitted('Metier', 'edit');
        if (isPermistted) {
