...
@Component({
  selector: 'app-paiement-manager-list-manager',
  standalone: false,
  templateUrl: './paiement-manager-list-manager.component.html'
})
export class PaiementManagerListManagerComponent implements OnInit {

    protected fileName = 'PaiementManager';
    ...
    constructor( private service: PaiementManagerManagerService  , private moyenPaiementService: MoyenPaiementManagerService, private statusPaiementService: StatusPaiementManagerService, private managerService: ManagerManagerService, @Inject(PLATFORM_ID) private platformId?) {
    ...
    public findPaginatedByCriteria() {
        this.service.findPaginatedByCriteria(this.criteria).subscribe(paginatedItems => {
            ...
            this.items = allItems.filter(item =>
                item.manager?.email === this.authService.authenticatedUser.email
            );
            this.totalRecords = this.items.length;
            this.selections = new Array<PaiementManagerDto>();
    ...
    public async edit(dto: PaiementManagerDto) {
    ...
    public async view(dto: PaiementManagerDto) {
        this.service.findByIdWithAssociatedList(dto).subscribe(res => {
            this.item = res;
    ...
    public async delete(dto: PaiementManagerDto) {
    ...
    public async duplicate(dto: PaiementManagerDto) {
    ...
    public exportPdf(dto: PaiementManagerDto): void {
...
// add
    ...
    public initCol() {
        this.cols = [
            ...
            {field: 'moyenPaiement?.libelle', header: 'Moyen paiement'},
            {field: 'statusPaiement?.libelle', header: 'Status paiement'},
            {field: 'manager?.email', header: 'Manager'},
        ];
    }


    public async loadMoyenPaiement(){
        this.moyenPaiementService.findAllOptimized().subscribe(moyenPaiements => this.moyenPaiements = moyenPaiements, error => console.log(error))
    }
    ...
    public async loadManager(){
        this.managerService.findAllOptimized().subscribe(managers => this.managers = managers, error => console.log(error))
    }


	public initDuplicate(res: PaiementManagerDto) {
	}
    ...
    public prepareColumnExport(): void {
        this.service.findByCriteria(this.criteria).subscribe(
            (allItems) =>{
                this.exportData = allItems.map(e => {
					return {
						...
						'Moyen paiement': e.moyenPaiement?.libelle ,
						'Status paiement': e.statusPaiement?.libelle ,
						'Manager': e.manager?.email ,
					}
				});
            ...
            //'Moyen paiement': this.criteria.moyenPaiement?.libelle ? this.criteria.moyenPaiement?.libelle : environment.emptyForExport ,
            //'Status paiement': this.criteria.statusPaiement?.libelle ? this.criteria.statusPaiement?.libelle : environment.emptyForExport ,
            //'Manager': this.criteria.manager?.email ? this.criteria.manager?.email : environment.emptyForExport ,
            }];
			}
        )
    }
    ...
    get selections(): Array<PaiementManagerDto> {
        return this.service.selections;
    }

    set selections(value: Array<PaiementManagerDto>) {
