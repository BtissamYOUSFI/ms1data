...
@RestController
@RequestMapping("/api/manager/paiementManager/")
public class PaiementManagerRestManager {
    ...
    @Operation(summary = "Finds a list of all paiementManagers by manager email")
    @GetMapping("email/{email}")
    public ResponseEntity<List<PaiementManagerDto>> findByManagerEmail(@PathVariable String email) throws Exception {
        ResponseEntity<List<PaiementManagerDto>> res = null;
        List<PaiementManager> list = service.findByManagerEmail(email);
        HttpStatus status = HttpStatus.NO_CONTENT;
            converter.initObject(true);
        List<PaiementManagerDto> dtos  = converter.toDto(list);
        if (dtos != null && !dtos.isEmpty())
   ...
   public PaiementManagerRestManager(PaiementManagerManagerService service, PaiementManagerConverter converter){
        this.service = service;
        this.converter = converter;
    }

    private final PaiementManagerManagerService service;
